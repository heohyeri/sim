...
def assign_clusters_to_robots(robot_list, centroids):
...
def assign_initial_target_clusters(robot_list, target_points):
    labels, centroids = run_kmeans(target_points, k=len(robot_list), seed=0)
    robot_cluster_map = assign_clusters_to_robots(robot_list, centroids)

    for robot_idx, robot in enumerate(robot_list):
        cluster_idx = robot_cluster_map.get(robot_idx)
        ...
        cluster_targets = [
            target_idx
            for target_idx, label in enumerate(labels)
            if label == cluster_idx
        ]
        robot_pos = np.squeeze(robot.state[0:2])
        robot.assigned_targets = order_target_indices(robot_pos, cluster_targets, target_points)

    return robot_cluster_map
...
def select_cluster_target(robot, pos, target_points, iteration):
    ...
    assigned_targets = getattr(robot, 'assigned_targets', [])
    nearby_indices = [
        idx for idx in assigned_targets
...
def select_nearest_target(robot, pos, target_points, iteration):
    blocked_targets = getattr(robot, 'blocked_targets', {})
    robot.blocked_targets = {
        idx: unblock_step
        for idx, unblock_step in blocked_targets.items()
...
assign_initial_target_clusters(env.robot_list, target_points)
...
for i in range(15000):
    ...
    for r_idx, robot in enumerate(env.robot_list):
        ...
        if target is None and assigned_unvisited:
            target_idx = select_cluster_target(robot, pos, target_points, i)
            if target_idx is not None:
                target = target_points[target_idx]
        elif target is None and len(robot.visited_points) < len(target_points):
            target_idx = select_nearest_target(robot, pos, target_points, i)
            if target_idx is not None:
                target = target_points[target_idx]
        elif target is None:
